...
class const_infinite_LitBitSetIterator {
  public:
    ...
    const_infinite_LitBitSetIterator(
        const const_infinite_LitBitSetIterator &mit)
        : ls(mit.ls), i(mit.i) {}
    inline const_infinite_LitBitSetIterator &operator++();
    inline const Lit operator*() const;
    bool operator==(const const_infinite_LitBitSetIterator &rhs) {
        assert(&ls == &(rhs.ls));
        return i == rhs.i;
    }
    bool operator!=(const const_infinite_LitBitSetIterator &rhs) {
...
class LitBitSet {
  public:
    ...
    const_infinite_LitBitSetIterator infinite_iterator() const {
        return const_infinite_LitBitSetIterator(*this, 0);
    }
...
inline void LitBitSet::clear() {
    set.clear();
    _size = 0;
}

inline const_infinite_LitBitSetIterator &
const_infinite_LitBitSetIterator::operator++() {
    if (!ls.size())
        return *this;
...
inline const_LitBitSetIterator &const_LitBitSetIterator::operator++() {
    assert(i < ls.physical_size());
    ++i;
    while ((i < ls.physical_size()) && !ls.get(index2literal(i)))
        ++i;
    return *this;
}
...
inline LitBitSet::const_iterator LitBitSet::end() const {
    return const_LitBitSetIterator(*this, physical_size());
}
inline LitBitSet::const_iterator LitBitSet::begin() const {
    return const_LitBitSetIterator(*this, 2);
}
